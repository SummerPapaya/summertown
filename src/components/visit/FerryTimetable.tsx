...
const ROWS = [
  { tide: 'Morning tide', out: '08:10', back: '09:00', golden: false },
  { tide: 'Midday tide', out: '12:20', back: '13:05', golden: false },
  { tide: 'Golden tide', out: '17:40', back: '18:25', golden: true },
];
...
export default function FerryTimetable() {
  ...
  const chooseGolden = () => {
    ...
    toast('Ah, a person of culture.', {
      description: 'The whole town just turned golden for you.',
    });
  };
    ...
    <section id="ferry" className="mx-auto max-w-[1200px] scroll-mt-[100px] px-6 py-24">
      <div className="text-center">
        ...
        <h2 className="mt-2 font-display text-[clamp(2rem,4vw,3.5rem)] font-semibold leading-[1.05] text-ink">
          <Words text="The M.V. Bellweather runs on tide-time." />
        </h2>
        <motion.p
      ...
      <div className="mt-12 flex flex-col items-center justify-center gap-8 lg:flex-row lg:items-start">
        ...
        >
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <p className="font-display text-lg font-semibold leading-tight text-ink">
                Summer Town Ferry Co.
              </p>
              <p className="text-xs font-bold text-ink-soft">
                Crossings daily, tide permitting. Tide always permits.
              </p>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[560px]" role="group" aria-label="Ferry timetable">
              {/* header row */}
              <div
                className={cn(
                  rowGrid,
              ...
              >
                <span>Tide</span>
                <span>Depart Mainland → Summer Town</span>
                <span>Depart Summer Town → Mainland</span>
                <span />
              </div>
                ...
                {ROWS.map((r, i) => {
                      ...
                      <span className="font-extrabold text-ink">
                        ...
                        {r.golden && (
                          <span className="mt-0.5 flex items-center gap-1 font-hand text-lg font-bold leading-none text-coral">
                            <Star className="h-3.5 w-3.5 fill-coral" /> the good one
                          </span>
                        )}
                      </span>
                  ...
                  return r.golden ? (
                    <motion.button
                      ...
                      title="Ride the golden tide — switches the whole town to Golden Hour"
                      className={cn(
                    ...
                    <motion.div key={r.tide} className={cn('vt-row bg-white/45', rowGrid)} {...motionProps}>
                      {Inner}
                    </motion.div>
                  );
                })}
              ...
              >
                <p className="text-sm font-bold text-ink-soft">
                  Starlight return <span className="font-display text-base text-ink">21:50</span> —
                  summer only, bells permitting.
                </p>
        ...
        >
          ...
          <h3 className="font-display text-xl font-semibold text-ink">Fares</h3>
          <p className="font-hand text-xl leading-none text-ink-soft">per crossing</p>
          <ul className="mt-4 space-y-3">
            {FARES.map(([who, price]) => (
              ...
              >
                <span className="text-sm font-extrabold text-ink">{who}</span>
                <span className="font-hand text-xl text-ink-soft">{price}</span>
              </li>
            ))}
          </ul>
          <p className="mt-4 font-hand text-lg leading-snug text-ink-soft">
            stories must be true, or at least well told
          </p>
        </motion.aside>
      </div>
    </section>
